...
export function detectDeviceType(name: string): import('@/types/bluetooth').DeviceType {
...
export function getChannels(deviceType: import('@/types/bluetooth').DeviceType): string[] {
  return deviceType === 'headphone' ? CHANNELS_HEADPHONE : CHANNELS_HEADBAND;
}

export function getScalingParams(deviceType: import('@/types/bluetooth').DeviceType) {
  switch (deviceType) {
...
export function getDeviceSettings(deviceType: import('@/types/bluetooth').DeviceType): import('@/types/bluetooth').DeviceSettings {
...
export function makeStartupSequence(settings: import('@/types/bluetooth').DeviceSettings): { cmd: Uint8Array; delay: number }[] {
...
export function makeImpedanceSequence(settings: import('@/types/bluetooth').DeviceSettings): { cmd: Uint8Array; delay: number }[] {
  const seq: { cmd: Uint8Array; delay: number }[] = [];
  seq.push({ cmd: new Uint8Array([COMMANDS.POWER_DOWN]), delay: TIMING.POWERDOWN_TO_IDLE });
  seq.push({ cmd: new Uint8Array([COMMANDS.GO_IDLE]), delay: TIMING.IDLE_TO_SETTINGS });
  if (settings.needsSettingsPacket) {
